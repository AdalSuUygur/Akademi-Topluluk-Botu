...
class CommunicationRequest(BaseModel):
    """İletişim isteği komutu için input validation."""
    
    topic: str = Field(..., description="İletişim isteği konusu")
    description: str = Field(default="", description="Detaylı açıklama")
    ...
    def validate_topic(cls, v: str) -> str:
        ...
        v = v.strip()
        if not v:
            raise ValueError('İletişim isteği konusu boş olamaz')
        if len(v) > 200:
            raise ValueError('Konu en fazla 200 karakter olabilir')
        return v
    ...
    @classmethod
    def parse_from_text(cls, text: str) -> 'CommunicationRequest':
        """
        /iletisim-kur komutundan gelen text'i parse eder.
        Format: /iletisim-kur [konu] [açıklama]
        """
        if not text:
            raise ValueError("İletişim isteği için en azından konu gerekli")
